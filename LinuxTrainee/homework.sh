...
function config_raid {
	...
	for u in "${MY_ARRAY[@]}"; do # creating path for disk input
...
function config_nfs {
	...
	while true; do
		...
		read mountPoint
		if [ "$?" = "0" ] | ls $mountPoint; then
			echo 'SUCCESS input mount point'
			break
		else
			echo 'Please input correct mount point(ls check).'
		fi
	done
...
7. exit'
	...
	if [[ $tempOption = 1 ]]; then
		...
		clear
		config_ssh
	elif [[ $tempOption = 3 ]]; then
		clear
		config_local_repo
	elif [[ $tempOption = 4 ]]; then
		clear
		config_raid
	elif [[ $tempOption = 5 ]]; then
		clear
		rollback_raid
	elif [[ $tempOption = 6 ]]; then
		clear
		config_nfs
	elif [[ $tempOption = 7 ]]; then
		clear
